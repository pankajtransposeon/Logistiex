...
// }, [acceptedArray]);
  ...
  const submitForm11 = () => {
        ...
        tx.executeSql(
          ...
          (tx, results) => {
            ...
            tx.executeSql(
              ...
              [bagSeal, route.params.userId+'-'+currentDate+'-'+(results.rows.length+1), currentDate, JSON.stringify(acceptedArray),'scanPending',route.params.consignorCode],
              (tx, results11) => {
    ...
    function CloseBag() {
      ...
      //     setBagSeal('');
      //     console.log('\n Data Added to local db successfully closebag');
      //     ToastAndroid.show('Bag successfully closed',ToastAndroid.SHORT);
      //     console.log(_res);
      //     viewDetailBag();
      //   }, error => {
      //     console.log('error on adding data ' + error.message);
      ...
      <Modal isOpen={showCloseBagModal11} onClose={() => { setShowCloseBagModal11(false); reloadScanner(); setScanned(true);}} size="lg">
        ...
        />
          ...
          backgroundColor:'white',
            ...
            <Button flex="1" mt={2} bg="#004aad" onPress={() => { CloseBagEndScan(); setShowCloseBagModal11(false);}}>Submit</Button>
          </Modal.Body>
        </Modal.Content>
      </Modal>
      ...
      {/* <Modal isOpen={showQRCodeModal} onClose={() => setShowQRCodeModal(false)} size="lg">
        ...
        <View>
          ...
          <View style={{width: '90%', flexDirection: 'row', justifyContent: 'space-between', alignSelf: 'center', marginTop: 10 }}>
            ...
            <Button w="48%" size="lg" bg={buttonColor} onPress={() => { if (acceptedArray.length === 0){
              // setCloseBagColor('gray.300');
