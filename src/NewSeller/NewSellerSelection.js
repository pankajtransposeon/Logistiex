        ...
        onPress={()=>setLine(false)}><Text style={{color:'blue'}}>{acc}</Text></TouchableOpacity>
          ...
          {/* <View style={styles.iconbar}>
            <TouchableOpacity style={[styles.scanbtn,{borderBottomColor:showline==true?'black':'white'}]}
              onPress={()=>setLine(true)} ><Text style={{color:'#000'}}>Pending  {route.params.Forward}</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.scanbtn2,{borderBottomColor:showline==false?'black':'white'}]} 
              onPress={()=>setLine(false)}><Text style={{color:'blue'}}>Completed {net}</Text></TouchableOpacity>
          </View> */}
...
//         await axios.get('https://bked.logistiex.com/SellerMainScreen/getSellerList/Tarun123')
//         .then((res) => {
//             setOrder(res.data)
//     }, (error) => {
//         alert(error);
